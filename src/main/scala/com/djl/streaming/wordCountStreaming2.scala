...
object wordCountStreaming2 {
  ...
  val updateFu = (it: Iterable[(String, Seq[Int], Option[Int])])=> {
    val ret: Iterable[(String, Int)] = it.map(t => (t._1.toString, t._2.sum + t._3.getOrElse(0)))
    ret
  }
  def main(args: Array[String]): Unit = {
    ...
    //参数1 ： 更新参数  ， 参数2 ： 自定义partition ， 参数3 ： 是否记忆，是否更新
     ...
     //words.map(x => (x, 1)).updateStateByKey[Int](updateFu, partitioner, true)
